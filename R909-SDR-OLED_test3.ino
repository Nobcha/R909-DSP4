...
// --=----=----=----=----=----=----=----=----=----=----=----=----
// 2024.02.08  V3.2  87% 42%
//  remaining issues
//  1.RSSi scanning noise 2.FM/AM volume level, 2. On SQ-ON mute noise
//
...
#define RESET_PIN 17
#define FM_STATION_FREQ 8020           // 80.2MHz - Select FM802
SI4735 rx;
...
#define led_pin A1     // Orange LED on panel 
// --=----=----=----=----=----=----=----=----=----=----=----=----
...
#define F_COR_AD 420    // frequency error correction value
// --=----=----=----=----=----=----=----=----=----=----=----=----
//
...
// --=----=----=----=----=----=----=----=----=----=----=----=----
// variables
boolean if_mode = 1;             // 1：LO mode FREQ being LO above
...
volatile char i = 0;
volatile unsigned char re_result = 0;    // Rotary switch activated
...
//  getSettings();
  ...
  if ( fstep == 1000 || fstep == 10000 || fstep == 100000 ||
    ...
    EEPROM.put( BAND_W_AD, 0);   // band with index preset on EPROM
...
void loop() {
  ...
  blink(300, 500);      // Blinking for monitor LED and FUNC display on OLED
...
void setFMfreq(uint16_t fm_freq){ // FM set for Si4732
...
//
  ...
  cmd_si5351(177,0xA0);    // Reset PLL_A.  This may be nesessary when M is changing
                           // (especialy from M=4 to M>=6 …)
}

// To write one byte datum in the resister
...
void Vol_Squ_Disp(void){      // To display VOL & SQU setting
...
void snr_rssi_Disp() {      // To display Si4732's SNR & RSSI
...
void s_meter_Disp()   {
  ...
  else  digitalWrite(led_pin, LOW);    // if rssi, led ON
}
...
// To display IF frequency 
  display.setTextSize(1);
  display.setCursor(92, 48);
  display.print(bandwidthAM[bandAM_idx]);
  display.print(F("kHz"));
//  display.print(mode);  
  ...
  else display.print(F(" LO")); // VFO or LO
...
void rotary_event(uint8_t dir) {         // Rotary encoder Up/Down 
 ...
 //       set21400kHz();               // Change to AM 21.4MHz mode
  ...
  //      set21400kHz();               // Change to AM 21.4MHz mode
    ...
    if (dir == 0x10) {               // Clockwise, increment  fstep_no
      bandAM_idx ++;
      if (bandAM_idx > 6) bandAM_idx = 6;
    }
    else if (dir == 0x20) {
      bandAM_idx --;
      if (bandAM_idx < 0) bandAM_idx = 0;
    }
    rx.setBandwidth(bandAM_idx,1);  // To effect the Si4732 AM band width
  }  
    ...
    if(BandSelect == true ){
      ...
      last_ffrequency0 = 0;       // To activate send_frequency function
      send_frequency(freq_0, 0);  //send it to si5351
    }
  }  
  ...
  else if (mode == FUNCTION) {   // Clockwise RE
    if (dir == 0x10){
      mode_temp ++;
      if (mode_temp > 9) mode_temp = 9; // 1-9   
    }
    else if (dir == 0x20) {      // Counter clockwise function
      mode_temp --;
      if (mode_temp < 1) mode_temp = 1; //
    }  
  }
